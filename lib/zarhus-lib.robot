...
Flash Zarhus OS
    ...
    Execute Command In Terminal Should Succeed
    ...    gunzip -c /tmp/zarhus.wic.gz | dd of=/dev/${device}
...
Install Zarhus OS
    ...
    ...    ${TARGET_DEVICE}, or /dev/sda if testing on QEMU and add to
    ...
    ...    === Requirements ===
...
Zarhus First Boot Setup
    ...
    Press Enter
    Read From Terminal Until Prompt
    Execute Command In Terminal With Sudo    reboot    wait_for_prompt=${FALSE}
...
Set Zarhus Features
    [Documentation]    Return features in global ``${FEATURES}`` variable as a
    ...    list. Features should be split with comma. Returns empty list if
