...
**Important**: To run OSFV tests on Zarhus OS with encryption feature enabled
you have to make sure that kernel command line contains `console=ttyS0,115200`
instead of `console=tty1`, as disk encryption password prompt displayed in
initramfs shows only on that `tty`. Running tests with wrong image will result
in tests or suite preparation steps failing due to timeout when waiting for
initramfs encryption password prompt.
One workaround to this would be to include manual step during this moment if
some flag is set, indicating no serial output in initramfs.
...
- `features` - list of features that tested Zarhus OS contains, split by
    ...
    - `encryption` - rootfs encryption feature
    - `otab` - A/B updates

    `hardening` feature is assumed always enabled as current tests and keywords
    use `user` account which is added in this feature. Without this feature
    only `debug` images could be tested (with enabled `root` account).
